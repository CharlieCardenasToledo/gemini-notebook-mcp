...
- Added a GitHub Release workflow for tokenless npm trusted publishing through OIDC,
  with npm provenance and no long-lived `NPM_TOKEN` secret.
- Added release metadata validation for the package name, stable version, exact tag,
  public access, and GitHub repository identity, with regression coverage.
- Added clean-install verification of the published registry package, including MCP
  startup, runtime version, and tool discovery.

### Changed

- npm publication is now gated by the full project checks, browser-independent MCP
  preflight, local package smoke test, and membership of the release commit in the
  default branch.
