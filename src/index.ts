 ...
 *
 * MCP Server for Google NotebookLM - grounded chat through NotebookLM
...
import { ArtifactStore } from "./notebooklm/artifact-store.js";
...
a NotebookLM notebook (chat using Google's NotebookLM-managed model grounded
on user-uploaded sources, ingest sources, generate Audio Overviews).
...
Use \`add_source\` for one text, website, or public YouTube source, or
\`batch_add_sources\` for several. Use \`list_sources\` to observe the live
indexing state instead of assuming a fixed delay.
...
The v2.3 persistent equivalent is \`generate_artifact(type="audio_overview")\`.
Keep its \`job_id\`, poll \`get_artifact_status\`, and finish with
\`download_artifact\`. Jobs survive MCP reconnects.

## Constraints

- Notebook and generation limits vary by account and plan; rely on the live
  Google interface rather than a hardcoded allowance.
- Session timeout: ~15 min idle (see \`get_health.session_timeout\`).
- File and Drive picker uploads are not yet exposed. Public YouTube URLs are.
- Audio Overview is the only Studio output currently exposed; Video,
...
class NotebookLMMCPServer {
  ...
  constructor() {
    ...
    const allTools = buildToolDefinitions() as Tool[];
  ...
  private createProtocolServer(): Server {
    ...
    this.setupHandlers(server, toolHandlers);
   ...
   */
  private setupHandlers(server: Server, toolHandlers: ToolHandlers): void {
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request, extra) => {
      ...
      const progressToken = extractProgressToken(rawArgs);
      ...
      try {
        ...
        switch (name) {
          case "ask_question":
            result = await toolHandlers.handleAskQuestion(
              args as {
                question: string;
                session_id?: string;
                notebook_id?: string;
          ...
          case "add_notebook":
            result = await toolHandlers.handleAddNotebook(
              args as {
                url: string;
                name: string;
                description: string;
          ...
          case "import_account_notebook":
            result = await toolHandlers.handleImportAccountNotebook(
              args as {
          ...
          case "sync_library":
            result = await toolHandlers.handleSyncLibrary(
              args as { apply?: boolean },
              extra.signal
            );
            break;

          case "get_notebook":
            result = await toolHandlers.handleGetNotebook(args as { id: string });
            break;

          case "select_notebook":
            result = await toolHandlers.handleSelectNotebook(args as { id: string });
            break;

          case "update_notebook":
            result = await toolHandlers.handleUpdateNotebook(
              args as {
                id: string;
                name?: string;
                description?: string;
          ...
          case "remove_notebook":
            result = await toolHandlers.handleRemoveNotebook(args as { id: string });
            break;

          case "search_notebooks":
            result = await toolHandlers.handleSearchNotebooks(args as { query: string });
            break;
          ...
          case "close_session":
            result = await toolHandlers.handleCloseSession(args as { session_id: string });
            break;
          ...
          case "cleanup_data":
            result = await toolHandlers.handleCleanupData(
              args as {
                confirm: boolean;
          ...
          case "add_source":
            result = await toolHandlers.handleAddSource(
              args as {
          ...
          case "list_sources":
            result = await toolHandlers.handleListSources(
              args as {
                session_id?: string;
                notebook_id?: string;
                notebook_url?: string;
          ...
          case "get_source_status":
            result = await toolHandlers.handleGetSource(
              args as {
                source_id?: string;
                name?: string;
                session_id?: string;
          ...
          case "batch_add_sources":
            result = await toolHandlers.handleBatchAddSources(
              args as {
                sources: Array<{
                  type: "url" | "text" | "youtube";
                  content: string;
          ...
          case "generate_artifact":
            result = await toolHandlers.handleGenerateArtifact(
              args as {
                type: "audio_overview";
          ...
          case "list_artifacts":
            result = await toolHandlers.handleListArtifacts(
              args as { notebook_id?: string; notebook_url?: string }
            );
            break;

          case "get_artifact_status":
            result = await toolHandlers.handleGetArtifactStatus(
              args as { job_id: string; show_browser?: boolean },
              extra.signal
            );
            break;

          case "download_artifact":
            result = await toolHandlers.handleDownloadArtifact(
              args as { job_id: string; destination_dir: string; show_browser?: boolean },
              extra.signal
            );
            break;

          case "generate_audio":
            result = await toolHandlers.handleGenerateAudio(
              args as {
                custom_prompt?: string;
                timeout_ms?: number;
                wait_for_completion?: boolean;
          ...
          case "get_audio_status":
            result = await toolHandlers.handleGetAudioStatus(
              args as {
                session_id?: string;
                notebook_id?: string;
                notebook_url?: string;
          ...
          case "download_audio":
            result = await toolHandlers.handleDownloadAudio(
              args as {
        ...
        const payload =
          result.success === false && result.error && !result.error_details
            ? { ...result, error_details: classifyError(result.error) }
            : result;
  ...
  async start(options: TransportOptions = { kind: "stdio" }): Promise<void> {
    ...
    log.diagnostic("Config directory", CONFIG.configDir);
    log.diagnostic("Data directory", CONFIG.dataDir);
...
async function main() {
  ...
  if (account) {
    ...
    log.info(`👤 Account profile active: ${hashLogValue(account)}`);
  }
  ensureDirectories();
  ...
  console.error("╔══════════════════════════════════════════════════════════╗");
  console.error("║                                                          ║");
  console.error(`║           NotebookLM MCP Server v${APP_VERSION.padEnd(24)}║`);
  console.error("║                                                          ║");
  console.error("║   Grounded chat through Google NotebookLM via MCP       ║");
  console.error("║                                                          ║");
  console.error("╚══════════════════════════════════════════════════════════╝");
  console.error("");
